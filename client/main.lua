...
local promptsRegistered = false
local insideCurtainZone = false
local curtainIsOpen = false

Config = Config or {}


Config.CurtainPromptZone = {
    coords = vector3(2548.7, -1305.2, 50.0),  
    radius = 4.0
}
...
CreateThread(function()
    while true do
        ...
        if promptsRegistered and insideCurtainZone then
            if IsPromptCompleted("open_curtain") and not curtainIsOpen then
                local curtain, curtainZone = GetShowCurtain("CANCAN_A")
                if curtain and curtain.animscene then
                    ...
                    if not _IsAnimSceneStarted(curtain.animscene) then
                    ...
                    TogglePrompts({ "open_curtain" }, false)
                    TogglePrompts({ "close_curtain" }, true)
                    curtainIsOpen = true
                end
            end

            if IsPromptCompleted("close_curtain") and curtainIsOpen then
                local curtain, curtainZone = GetShowCurtain("CANCAN_A")
                if curtain and curtain.animscene then
                    ...
                    TogglePrompts({ "close_curtain" }, false)
                    TogglePrompts({ "open_curtain" }, true)
                    curtainIsOpen = false
                end
            end
        end
...
CreateThread(function()
    while true do
        ...
        local dist = #(playerCoords - Config.CurtainPromptZone.coords)

        if dist <= Config.CurtainPromptZone.radius and not insideCurtainZone then
            insideCurtainZone = true
        ...
        elseif dist > Config.CurtainPromptZone.radius and insideCurtainZone then
            insideCurtainZone = false
            TogglePrompts("ALL", false)
        end
...
function StartShow(show, projection, movie)
    if show:upper() == "MOVIE" then
        if not IsMovieValid(projection, movie) then
            RSGCore.Functions.Notify('Invalid movie or projection.', 'error')
            return
        end
        projection = Config.Projections[projection]
        movie = Config.Movies[movie]
    end

    if not Config.Shows[show] then
        RSGCore.Functions.Notify('Invalid show selection.', 'error')
        return
    end

    local showData = Config.Shows[show]
    local animscene = Citizen.InvokeNative(0x1FCA98E33C1437B3, showData.animScene, 0, "Show Scene", false, true)

    if not animscene then
        RSGCore.Functions.Notify('Failed to create animscene!', 'error')
        return
    end

    LoadAnimScene(animscene)
    while not IsAnimSceneLoaded(animscene, true) do 
        Citizen.Wait(10)
    end
    ...
    for _, entity in pairs(showData.entities) do
        ...
        local ped = CreatePed(entity.model, entity.coords, entity.heading, false, true)
        if DoesEntityExist(ped) then
            SetEntityInvincible(ped, true)
            SetBlockingOfNonTemporaryEvents(ped, true)
            SetPedCanRagdoll(ped, false)
            SetAnimSceneEntity(animscene, entity.name, ped, 0) 
        else
    ...
    while not IsAnimSceneStarted(animscene, false) do 
        Citizen.Wait(100)
    end
...
function OpenTheatreMenu()
    local options = {}

    
    for showName, _ in pairs(Config.Shows) do
        table.insert(options, {
            title = showName:gsub("_", " "),
            description = 'Watch ' .. showName:gsub("_", " "),
            icon = 'fa-solid fa-theater-masks',
            onSelect = function()
                TriggerServerEvent('rdr-theatre:startSelectedShow', showName)
            end
        })
    end
    ...
    lib.registerContext({ 
        id = 'theatre_menu', 
        title = 'Saint Denis Theatre', 
        options = options 
    })
    lib.showContext('theatre_menu')
end



RegisterCommand('theatre', function()
    OpenTheatreMenu()
end)
